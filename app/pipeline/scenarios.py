...
def load_bess_scenario(name_or_path: str, storage: Storage | None = None) -> BessScenario:
    ...
    If `scenarios/bess/{name_or_path}.yaml` exists (relative to the repo
    root, or under `storage` if given), load it from there. Otherwise treat
    `name_or_path` as a literal filesystem path.
    """
    library = storage or get_storage(str(_REPO_ROOT))
