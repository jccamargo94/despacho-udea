...
class DataLoader:
    ...
    def load_csv(self) -> None:
        self.dispo = pd.read_csv(f"{self._folder}/dispo_declarada.csv", parse_dates=["datetime"], engine="pyarrow")
        self.ofertas = pd.read_csv(f"{self._folder}/ofertas.csv", parse_dates=["Date"], engine="pyarrow")
        self.demanda = pd.read_csv(f"{self._folder}/demaCome.csv", parse_dates=["datetime"], engine="pyarrow")
        self.agc_asignado = pd.read_csv(f"{self._folder}/agc_asignado.csv", parse_dates=["datetime"], engine="pyarrow")
        return None
    ...
    def load_txt_files(self) -> None:
        output = []
        MO = []
        with open(f"{self._folder}/OFEI{self._dispatch_date.month:0>2}{self._dispatch_date.day:0>2}.txt", "r") as file:
        ...
        self.minimo_operativo = self.minimo_operativo.set_index(["resource", "type"]).stack().reset_index()
        self.minimo_operativo.columns = ["resource", "type", "hour", "self.minimo_operativo"]
        self.minimo_operativo["datetime"] = pd.to_datetime(self._dispatch_date) + pd.to_timedelta(self.minimo_operativo["hour"], unit="h")
        self.minimo_operativo["minimo_operativo"] = self.minimo_operativo["minimo_operativo"].astype(float)
        # Condiciones Iniciales
        with open(f"data/dCondIniP{self._dispatch_date.month:0>2}{self._dispatch_date.day:0>2}.txt", "r") as file:
    ...
    def filter_data(self,) -> None:
        ...
        self.agc_asignado = self.agc_asignado[self.agc_asignado["datetime"].dt.date == self._dispatch_date]
